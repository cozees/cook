...
func TestDir(t *testing.T) {
	...
	expect(true, path)
	_, err := fmkdir.Apply([]string{path})
	assert.NoError(t, err)
	expect(false, path)
	...
	_, err = fmkdir.Apply([]string{pathRecur})
	assert.Error(t, err)
	expect(true, pathRecur) // expect no folder created
	_, err = fmkdir.Apply([]string{"-p", pathRecur})
	assert.NoError(t, err)
	expect(false, pathRecur)
	// create multiple input
	_, err = fmkdir.Apply(paths)
	assert.NoError(t, err)
	expect(false, paths[1:]...)
	// rmdir
	_, err = frmdir.Apply([]string{path})
	assert.Error(t, err)
	expect(false, path)
	_, err = frmdir.Apply([]string{pathRecur})
	assert.NoError(t, err)
	expect(true, pathRecur)
	p2dir := filepath.Dir(pathRecur)
	_, err = frmdir.Apply([]string{"-p", p2dir})
	assert.NoError(t, err)
	expect(true, path)
	expect(true, p2dir)
	_, err = frmdir.Apply(paths)
	assert.NoError(t, err)
	expect(true, paths[1:]...)
}
...
func TestRm(t *testing.T) {
	...
	assert.NoError(t, err)
	f := GetFunction("rm")
	_, err = f.Apply([]string{f1b})
	assert.NoError(t, err)
	_, err = os.Stat(f1b)
	assert.ErrorIs(t, err, os.ErrNotExist)
	_, err = f.Apply([]string{a})
	assert.Error(t, err)
	_, err = f.Apply([]string{"-r", a})
	assert.NoError(t, err)
...
func TestChown(t *testing.T) {
	...
	_, err = fn.Apply([]string{"-n", uid + ":" + gid, f})
	assert.NoError(t, err)
	_, err = fn.Apply([]string{"-n", uid, f})
	assert.NoError(t, err)
	_, err = fn.Apply([]string{"-n", ":" + gid, f})
	assert.NoError(t, err)
	// user and group name
	...
	assert.NoError(t, err)
	_, err = fn.Apply([]string{"-n", user.Username + ":" + group.Name, f})
	assert.Error(t, err)
	_, err = fn.Apply([]string{user.Username + ":" + group.Name, f})
	assert.NoError(t, err)
}
...
func TestChmod(t *testing.T) {
	...
	expect(c, "-rwx-wx---")
	f := GetFunction("chmod")
	_, err := f.Apply([]string{"go+x", a})
	assert.NoError(t, err)
	expect(a, "-rwx--x--x")
	expect(b, "-rwx--x---")
	expect(c, "-rwx-wx---")
	_, err = f.Apply([]string{"u-x,g-wx,o+r", b, c})
	assert.NoError(t, err)
	expect(a, "-rwx--x--x")
	expect(b, "-rw----r--")
	expect(c, "-rw----r--")
...
func TestCopy(t *testing.T) {
	...
	// test copy file
	_, err := fn.Apply([]string{fcp1, fcp31})
	assert.NoError(t, err)
	expectContent(fcp31, data1)
	_, err = fn.Apply([]string{fcp1, fcp31})
	assert.NoError(t, err)
	_, err = fn.Apply([]string{fcp2, fcp3, d})
	assert.NoError(t, err)
	assert.FileExists(t, fcp32)
	assert.FileExists(t, fcp33)
	// test copy folder
	_, err = fn.Apply([]string{d, d2})
	assert.Error(t, err)
	_, err = fn.Apply([]string{"-R", d, d2})
	assert.NoError(t, err)
